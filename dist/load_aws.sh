...
ERRLOG=$INPUTDIR/logs/error.log-aws
export ERRLOG
...
echo "load_aws.sh               : start"
# fetch API data dump over HTTP 
echo "aws-api2json.pl           : collect data from AWS, "`date`
$PERL -w $BINDIR/aws-api2json.pl 2>>$ERRLOG

echo "aws-json2rrd.pl           : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/aws-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "aws-json2rrd.pl finish     : status $status"
...
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "aws-json2db.pl             : db, "`date`
    $PERL -w $BINDIR/aws-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/aws-cleanup.pl 2>>$ERRLOG
