...
ERRLOG=$INPUTDIR/logs/error.log-ovirt
...
#
# Store data from oVirt PostgreSQL DB to rrd files
#

echo "load_ovirt.sh     : start"
echo "ovirt-db2json.pl  : collect data from db, "`date`
$PERL -w $BINDIR/ovirt-db2json.pl 2>>$ERRLOG

echo "ovirt-json2rrd.pl : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/ovirt-json2rrd.pl 2>>$ERRLOG & wait $!    # start the job wait for it and save its return value
status=$?
echo "ovirt-json2rrd.pl : status $status, "`date`
kill -ALRM $pid_wait_time    # send ALRM signal to watchit
wait $pid_wait_time          # wait for watchit to finish cleanup

# [optional] fetch data from REST API
if [ ! "$USE_OVIRT_RESTAPI"x = "x" ]; then
  if [ $USE_OVIRT_RESTAPI -eq 1 ]; then
    $PERL -w $BINDIR/ovirt-api2json.pl 2>>$ERRLOG
  fi
fi

# generate html configuration pages
$PERL -w $BINDIR/ovirt-genconf.pl 2>>$ERRLOG

# remove VMs data if they are older than 3 months
$PERL -w $BINDIR/ovirt-cleanup.pl 2>>$ERRLOG

# XorMon load data to SQLite
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    $PERL -w $BINDIR/ovirt-json2db.pl 2>>$ERRLOG
  fi
fi
...
if [ $ERR_TOT -gt 0 ]; then
  echo "An error occured in lpar2rrd, check $ERRLOG and output of load_ovirt.sh"
  echo ""
