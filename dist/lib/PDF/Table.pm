...
# deprecated names) won't modify original %arg hash on the outside.
    ...
    my $default_text    = ( defined $arg{'default_text'} ) ? $arg{'default_text'} : $empty_cell_text; # KZ: adjusted for perl <5.010
        ...
        while (scalar(@{$data}) and $cur_y-$rows_height->[0] > $bot_margin) {
            ...
            for ( my $col_idx = 0; $col_idx < $columns_number; $col_idx++ ) {
               ...
               #}
                ...
                $data_row->[$col_idx] = $cell_def_text unless ( defined $data_row->[$col_idx] ); # KZ: adjusted for perl <5.010
