...
ERRLOG=$INPUTDIR/logs/error.log-xen
export ERRLOG
...
#
# Store data from XenServer to rrd files
#

echo "load_xenserver.sh      : start"

# fetch XAPI data dump over HTTP and metadata over SSH
echo "xen-xapi2json.pl       : collect data from XAPI, "`date`
$PERL -w $BINDIR/xen-xapi2json.pl 2>>$ERRLOG 

echo "xen-json2rrd.pl        : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/xen-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "xen-json2rrd.pl finish : status $status"
...
$PERL -w $BINDIR/xen-cleanup.pl 2>>$ERRLOG

# XorMon load data to SQLite
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    $PERL -w $BINDIR/xen-json2db.pl 2>>$ERRLOG 
  fi
fi
