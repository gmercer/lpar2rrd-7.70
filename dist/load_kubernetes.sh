...
ERRLOG=$INPUTDIR/logs/error.log-kubernetes
export ERRLOG
...
echo "load_kubernetes.sh        : start"

# fetch API data dump over HTTP 
echo "kubernetes-api2json.pl    : collect data from Kubernetes, "`date`
$PERL -w $BINDIR/kubernetes-api2json.pl 2>>$ERRLOG

echo "kubernetes-csv2rrd.pl     : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/kubernetes-csv2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "kubernetes-csv2rrd.pl    : status $status"
kill -ALRM $pid_wait_time    # send ALRM signal to watchit
wait $pid_wait_time          # wait for watchit to finish cleanup

echo "kubernetes-background.pl  : start background task, "`date`
#$PERL -w $BINDIR/kubernetes-background.pl 2>>$ERRLOG &
$PERL -w $BINDIR/kubernetes-background.pl 2>>$ERRLOG

# XorMon load data to SQLite
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "kubernetes-json2db.pl     : db, "`date`
    $PERL -w $BINDIR/kubernetes-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/kubernetes-cleanup.pl 2>>$ERRLOG
