...
ERRLOG=$INPUTDIR/logs/error.log-azure
export ERRLOG
...
#
# Store data from Azure to rrd files
#

echo "load_azure.sh                : start"
# fetch API data dump over HTTP 
echo "azure-api2json.pl            : collect data from Microsoft Azure, "`date`
$PERL -w $BINDIR/azure-api2json.pl 2>>$ERRLOG

echo "azure-json2rrd.pl            : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/azure-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "azure-json2rrd.pl finish     : status $status"
...
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "azure-json2db.pl             : db, "`date`
    $PERL -w $BINDIR/azure-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/azure-cleanup.pl 2>>$ERRLOG
