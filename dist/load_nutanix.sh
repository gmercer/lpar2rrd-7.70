...
ERRLOG=$INPUTDIR/logs/error.log-nutanix
export ERRLOG
...
#
# Store data from Nutanix Server to rrd files
#

echo "load_nutanix.sh           : start"

# fetch API data dump over HTTP 
echo "nutanix-api2json.pl       : collect data from API, "`date`
$PERL -w $BINDIR/nutanix-api2json.pl 2>>$ERRLOG

echo "nutanix-json2rrd.pl       : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/nutanix-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "nutanix-json2rrd.pl end   : status $status"
...
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "nutanix-json2db.pl        : db, "`date`
    $PERL -w $BINDIR/nutanix-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/nutanix-cleanup.pl 2>>$ERRLOG
