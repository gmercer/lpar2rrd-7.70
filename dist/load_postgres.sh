...
ERRLOG=$INPUTDIR/logs/error.log-postgres
...
POSTGRES_LIST=`$PERL $BINDIR/postgres-list.pl`
if [ "$POSTGRES_LIST" = "NO_POSTGRES_HOSTS_FOUND" ]; then
  echo No hosts were found in $INPUTDIR/etc/web_config/hosts.json to run PostgreSQL. Check Host Configuration in GUI and configure your databases there.
  exit;
fi

#HS reset
HS_DIR=$INPUTDIR/tmp/health_status_summary/PostgreSQL
...
for MYDB in $POSTGRES_LIST
   do
  export MYDB=$MYDB
  echo Start $MYDB Data Fetch

  $PERL -w $BINDIR/postgres-db2json.pl $MYDB 2>>$ERRLOG


  echo "postgres-json2rrd.pl : push data to rrd, "`date`

  $PERL -w $BINDIR/postgres-json2rrd.pl $MYDB 2>>$ERRLOG
  echo "postgres-json2rrd.pl : status $status, "`date`

  # generate html configuration pages
  $PERL -w $BINDIR/postgres-genconf.pl $MYDB & 2>>$ERRLOG
done
  ...
  if [ ! "$XORMON"x = "x" ]; then
    if [ "$XORMON" != "0" ]; then
      ...
      $PERL -w $BINDIR/postgres-json2db.pl 2>>$ERRLOG
    fi
  fi
...
if [ $ERR_TOT -gt 0 ]; then
  echo "An error occured in lpar2rrd, check $ERRLOG and output of load_postgres.sh"
  echo ""
