...
ERRLOG=$INPUTDIR/logs/error.log-fusioncompute
export ERRLOG
...
#
# Store data from Proxmox to rrd files
#

echo "load_fusioncompute.sh     : start"

# fetch API data dump over HTTP 
echo "fusioncompute-api2json.pl : collect data from Fusion Compute API, "`date`
$PERL -w $BINDIR/fusioncompute-api2json.pl 2>>$ERRLOG

echo "fusioncompute-json2rrd.pl : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/fusioncompute-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "fusioncompute-json2rrd.pl : status $status"
...
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "fusioncompute-json2db.pl  : db, "`date`
    $PERL -w $BINDIR/fusioncompute-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/fusioncompute-cleanup.pl 2>>$ERRLOG
