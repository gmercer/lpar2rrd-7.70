...
ERRLOG=$INPUTDIR/logs/error.log-oracledb
...
#
# Store data from OracleDB to rrd files
#
ODB_LIST=`$PERL $BINDIR/oracleDB-list.pl`
if [ "$ODB_LIST" = "NO_ORACLEDB_HOSTS_FOUND" ]; then
  echo No hosts were found in $INPUTDIR/etc/web_config/hosts.json to run OracleDB. Check Host Configuration in GUI and configure your databases there.
  exit;
fi

#HS reset
HS_DIR=$INPUTDIR/tmp/health_status_summary/OracleDB
...
for MYDB in $ODB_LIST
  do
  export MYDB=$MYDB
  echo Start $MYDB Data Fetch

  PROC=`ps -ef | egrep 'oracleDB-perfWrapper.sh *'$MYDB' *$' | grep -v grep | wc -l|sed 's/ //g'`
  if [ $PROC -lt 3 ]; then
    $BINDIR/oracleDB-perfWrapper.sh $BINDIR $MYDB 2>>$ERRLOG &
  else
    date_act=`date "+%Y-%m-%d_%H:%M"`
    echo "$0: $date_act : Command oracleDB-perfWrapper.sh is already running for $MYDB, cannot start the next one"
    echo "$0: $date_act : Command oracleDB-perfWrapper.sh is already running for $MYDB, cannot start the next one"  2>>$ERRLOG
  fi

  PROC=`ps -ef | egrep 'oracleDB-db2conf.pl *'$MYDB' *$' | grep -v grep | wc -l|sed 's/ //g'`
  if [ $PROC -lt 1 ]; then
    $PERL -w $BINDIR/oracleDB-db2conf.pl $MYDB 2>>$ERRLOG &
    echo "oracleDB-db2conf.pl : status $status, "`date`

  else
    date_act=`date "+%Y-%m-%d_%H:%M"`
    echo "$0: $date_act : Command oracleDB-db2conf.pl is already running for $MYDB, cannot start the next one"
    echo "$0: $date_act : Command oracleDB-db2conf.pl is already running for $MYDB, cannot start the next one"  2>>$ERRLOG
  fi

  # generate html configuration pages
  $PERL -w $BINDIR/oracleDB-genconf.pl $MYDB & 2>>$ERRLOG 

  # XorMon load data to SQLite
  if [ ! "$XORMON"x = "x" ]; then
    if [ "$XORMON" != "0" ]; then
      echo "oracleDB-json2db.pl             : db, "`date`
      $PERL -w $BINDIR/oracleDB-json2db.pl 2>>$ERRLOG
    fi
  fi
...
if [ $ERR_TOT -gt 0 ]; then
  echo "An error occured in lpar2rrd, check $ERRLOG and output of load_oracledb.sh"
  echo ""
