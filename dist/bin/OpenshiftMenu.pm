# OpenshiftMenu.pm
# Openshift-specific wrapper for Menu.pm (WIP)

package OpenshiftMenu;
...
sub create_page {
  ...
  my $hash = substr( md5_hex("os-$title-$last"), 0, 7 );
...
sub get_url {
  ...
  use Menu;
  my $menu = Menu->new('openshift');

  my $id = '';
  foreach my $param ( keys %{$params} ) {
    if ( $param =~ /(namespace|node|project|cluster|pod|service|container)/ ) {
      $id = $params->{$param};
      last;
    }
  }
