...
sub create_page {
  ...
  my $hash = substr( md5_hex("odb-$title-$last"), 0, 7 );
    ...
    if ( $args->{id} ) {
      ...
      if ( $page_type->{type} eq $args->{type} ) {
        $url =
            $page_type->{url_base} =~ /\.html$/
          ? $page_type->{url_base}
          : "$page_type->{url_base}?platform=$page_type->{platform}&type=$page_type->{type}&host=$host&server=$server";
        last;
      }
    }
  }
