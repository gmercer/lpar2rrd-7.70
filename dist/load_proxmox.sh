...
ERRLOG=$INPUTDIR/logs/error.log-proxmox
export ERRLOG
...
echo "load_proxmox.sh           : start"

# fetch API data dump over HTTP 
echo "proxmox-api2json.pl       : collect data from Proxmox API, "`date`
$PERL -w $BINDIR/proxmox-api2json.pl 2>>$ERRLOG

echo "proxmox-json2rrd.pl       : push data to rrd, "`date`

wait_time $timeout& pid_wait_time=$!         #start the timeout
trap "cleanup" ALRM          # cleanup after timeout

$PERL -w $BINDIR/proxmox-json2rrd.pl 2>>$ERRLOG & wait $!           # start the job wait for it and save its return value
status=$?
echo "proxmox-json2rrd.pl       : status $status"
...
if [ ! "$XORMON"x = "x" ]; then
  if [ "$XORMON" != "0" ]; then
    echo "proxmox-json2db.pl        : db, "`date`
    $PERL -w $BINDIR/proxmox-json2db.pl 2>>$ERRLOG
  fi
fi

# remove RRDs if they are older than 3 months and currently not reported by any host either
$PERL -w $BINDIR/proxmox-cleanup.pl 2>>$ERRLOG
